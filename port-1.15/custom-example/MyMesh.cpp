...
static void fill_contact_ref(mc_overlay_contact_ref_t& ref, const ContactInfo &from) {
  memset(&ref, 0, sizeof(ref));
  memcpy(ref.contact_prefix, from.id.pub_key, 6);
  strncpy(ref.title, from.name, sizeof(ref.title) - 1);
}

void MyMesh::overlayDirectMessage(const ContactInfo &from, uint32_t sender_timestamp, const char *text, bool acknowledged) {
  mc_overlay_contact_ref_t ref;
  fill_contact_ref(ref, from);
  mc_overlay_receive_direct_message(&ref, sender_timestamp, text, acknowledged);
}

void MyMesh::overlayRepeaterStatus(const ContactInfo &from, const uint8_t *data, uint8_t len) {
  uint8_t prefix[6];
  memset(prefix, 0, sizeof(prefix));
  memcpy(prefix, from.id.pub_key, 6);
  mc_repeater_admin_dispatch_status(prefix, 0, data, len);
}

bool MyMesh::importInviteToChannel(uint8_t channel_idx, const char* invite_text) {
  mc_channel_invite_t invite;
  ChannelDetails channel;
  if (!invite_text || !mc_overlay_try_import_channel_invite(invite_text, &invite)) return false;
  memset(&channel, 0, sizeof(channel));
  StrHelper::strncpy(channel.name, invite.name, sizeof(channel.name));
  if (!mc_channel_bridge_invite_to_secret(&invite, channel.channel.secret)) return false;
  if (!setChannel(channel_idx, channel)) return false;
  saveChannels();
  return true;
}

void MyMesh::overlayChannelMessage(const mesh::GroupChannel &channel, uint32_t timestamp, const char *text) {
  mc_overlay_channel_ref_t ref;
  ChannelDetails details;
  memset(&ref, 0, sizeof(ref));
  ref.channel_index = (uint8_t)findChannelIdx(channel);
  if (getChannel(ref.channel_index, details)) {
    strncpy(ref.title, details.name, sizeof(ref.title) - 1);
    ref.is_private = details.channel.secret[0] != 0;
  } else {
    strncpy(ref.title, "Channel", sizeof(ref.title) - 1);
  }
  mc_overlay_receive_channel_message(&ref, timestamp, text);
}
...
void MyMesh::onMessageRecv(const ContactInfo &from, mesh::Packet *pkt, uint32_t sender_timestamp,
  ...
  overlayDirectMessage(from, sender_timestamp, text, false);
...
void MyMesh::onSignedMessageRecv(const ContactInfo &from, mesh::Packet *pkt, uint32_t sender_timestamp,
  ...
  dirty_contacts_expiry = futureMillis(LAZY_CONTACTS_WRITE_DELAY);
  overlayDirectMessage(from, sender_timestamp, text, true);
  queueMessage(from, TXT_TYPE_SIGNED_PLAIN, pkt, sender_timestamp, sender_prefix, 4, text);
}

void MyMesh::onChannelMessageRecv(const mesh::GroupChannel &channel, mesh::Packet *pkt, uint32_t timestamp,
                                  const char *text) {
  overlayChannelMessage(channel, timestamp, text);
  int i = 0;
  if (app_target_ver >= 3) {
...
void MyMesh::onContactResponse(const ContactInfo &contact, const uint8_t *data, uint8_t len) {
  if (len >= 24) {
    overlayRepeaterStatus(contact, data, len);
  }
  uint32_t tag;
  memcpy(&tag, data, 4);
...
void MyMesh::handleCmdFrame(size_t len) {
  ...
  } else if (cmd_frame[0] == CMD_SET_CHANNEL && len > 2 && (char)cmd_frame[2] == 'm') {
    uint8_t channel_idx = cmd_frame[1];
    const char* invite_text = (const char*)&cmd_frame[2];
    if (importInviteToChannel(channel_idx, invite_text)) {
      writeOKFrame();
    } else {
      writeErrFrame(ERR_CODE_ILLEGAL_ARG);
    }
